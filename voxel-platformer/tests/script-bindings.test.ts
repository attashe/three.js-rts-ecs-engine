...
test('npc bindings forward attack/die/exists/list through the facade', () => {
    ...
    const npc: NpcFacade = {
        ...
        list() { return [...live] },
        setWaypoints(id, points) { calls.push({ type: 'setWaypoints', id, points }); return live.has(id) },
        goTo(id, point) { calls.push({ type: 'goTo', id, point }); return live.has(id) },
        ...
        setThreatMemory(id, seconds) { calls.push({ type: 'setThreatMemory', id, seconds }); return live.has(id) },
    }
