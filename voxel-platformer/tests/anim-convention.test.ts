...
test('combat clip set extends base locomotion with action and death clips', () => {
    ...
    for (const id of ['attack', 'spearAttack', 'attackWide', 'staffAttack', 'hammerAttack', 'shoot', 'shieldBlock', 'die', 'dead'] as const) {
    ...
    assert.equal(partial.ok, false)
    assert.deepEqual(partial.missing.sort(), ['attack', 'attackWide', 'dead', 'die', 'hammerAttack', 'shieldBlock', 'shoot', 'spearAttack', 'staffAttack'])
})
