...
test('Phase 12 underground mine stress is visible in procedural level exports', () => {
    ...
    assert.equal(level.runtimeMeta.name, 'Abandoned Mine Shafts')
    assert.equal(restored.metadata.name, 'Abandoned Mine Shafts')
    assert.equal(level.runtimeMeta.railCarts.length, 3)
    assert.equal(restored.metadata.railCarts?.length, 3)
    assert.equal(level.runtimeMeta.railCarts.filter((cart) => cart.enabled).map((cart) => cart.id).join(','), 'main_route_cart,bunk_branch_cart,ore_branch_cart')
