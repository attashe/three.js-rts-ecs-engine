...
test('demo quest: keeper trade option opens the supply shop', async () => {
    ...
    assert.equal(request.items?.[2]?.id, 'high-jump-boots')
    assert.equal(request.items?.[2]?.resource, 'high-jump-boots')
    assert.equal(request.items?.[2]?.unitSize, 1)
    assert.equal(request.items?.[3]?.id, 'high-speed-boots')
    assert.equal(request.items?.[3]?.resource, 'high-speed-boots')
