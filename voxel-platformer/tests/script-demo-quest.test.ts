...
test('demo quest: keeper trade option opens the supply shop', async () => {
    ...
    assert.equal(request.items?.[2]?.id, 'mana-potion')
    assert.equal(request.items?.[2]?.resource, 'mana-potion')
    assert.equal(request.items?.[2]?.unitSize, 1)
    assert.equal(request.items?.[3]?.id, 'high-jump-boots')
    assert.equal(request.items?.[3]?.resource, 'high-jump-boots')
    assert.equal(request.items?.[3]?.unitSize, 1)
    assert.equal(request.items?.[4]?.id, 'high-speed-boots')
    assert.equal(request.items?.[4]?.resource, 'high-speed-boots')
    assert.equal(request.items?.[4]?.unitSize, 1)
