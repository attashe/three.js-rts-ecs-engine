...
import { isGrounded, sweepAxis, type AABB, type ObstacleSource } from '../src/engine/voxel/voxel-collide'
import { ObstacleRegistry } from '../src/engine/ecs/obstacle-registry'
import { BLOCK, DEFAULT_PALETTE } from '../src/engine/voxel/palette'
import { createDynamicCollisionSystem } from '../src/engine/ecs/systems/dynamic-collision-system'
import { createPhysicsSystem } from '../src/engine/ecs/systems/physics-system'
import { BoxCollider, MovementState, PlayerControlled, Position, Velocity } from '../src/engine/ecs/components'
import { MovementStateId } from '../src/engine/ecs/movement-state'
...
test('DynamicCollisionSystem: overlapping players are marked Blocked via MovementState', () => {
    // (Trimmed from the parent engine — the MoveAlongPath path-release branch
    // is dropped in the platformer foundation. Just verify the player-actor
    // pair separation still flags MovementState.Blocked.)
    const chunks = new ChunkManager(DEFAULT_PALETTE)
    const world = createGameWorld()
    const a = addMovableActor(world, 0, 0, 0)
    const b = addMovableActor(world, 0.15, 0, 0)
    addComponent(world, a, MovementState)
    addComponent(world, b, MovementState)

    createDynamicCollisionSystem(chunks, { passes: 1, padding: 0.08 }).update(world, 1 / 60)

    // One of the two yielding entities gets the Blocked mark per the deterministic
    // tie-break in dynamic-collision-system.chooseYieldingEntity.
    const blocked = (MovementState.value[a] === MovementStateId.Blocked) ||
        (MovementState.value[b] === MovementStateId.Blocked)
