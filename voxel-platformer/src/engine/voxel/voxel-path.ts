...
export interface PathOptions {
    ...
    /** Optional runtime blocker for dynamic bodies such as characters/NPCs. */
 ...
 * alone if start === goal.
 *
 * Ported from the parent ARPG engine unchanged — it reads only voxel traits via
 * the (shared) palette helpers, so it works against the platformer's palette as
 * long as walkable blocks expose `pathSurface`. Coordinate convention: paths are
 * arrays of voxel-coord cells; add `(0.5, 0, 0.5)` for world-space waypoints so
 * the walker tracks the cell centre.
 */
