...
- `level-from-meta.ts` translates the editor's metadata into the runtime
  shape (pickups → coinPiles, pistons → PistonMechanismConfig, stones →
  physics stones / falling-stone spawners).
...
The Stones tab exposes two placement tools:

- **Place Stone** adds a direct physics stone that spawns at level start.
- **Place Spawner** adds a falling-stone emitter with tier, size, velocity,
  interval, initial delay, jitter, enabled state, and per-spawner max-active
  cap. Script ids are generated by default so the script engine can spawn /
  remove direct stones and enable, disable, or trigger authored spawners.
