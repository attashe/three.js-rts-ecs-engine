...
export function createCoinPile(): Group {
    ...
    for (let i = 0; i < 7; i++) {
        const coin = new Mesh(sharedCylinderGeometry(0.1, 0.1, 0.025, 14), gold)
        coin.name = `Coin${i + 1}`
...
export function createHealthPotion(): Group {
    ...
    const bottle = new Mesh(sharedSphereGeometry(0.16, 14, 10), glass)
    bottle.name = 'Bottle'
    bottle.scale.set(0.8, 1, 0.8)
    bottle.position.y = 0.18
    root.add(bottle)

    const fill = new Mesh(sharedSphereGeometry(0.13, 12, 8), liquid)
    fill.name = 'Liquid'
    fill.scale.set(0.78, 0.55, 0.78)
    fill.position.y = 0.13
    root.add(fill)

    const neck = new Mesh(sharedCylinderGeometry(0.055, 0.065, 0.16, 10), glass)
    neck.name = 'Neck'
    neck.position.y = 0.36
    root.add(neck)

    const stopper = new Mesh(sharedCylinderGeometry(0.06, 0.055, 0.08, 10), cork)
...
export function createTrainingDummy(): Group {
    ...
    const post = new Mesh(sharedCylinderGeometry(0.07, 0.08, 1.25, 10), wood)
    post.name = 'Post'
    post.position.y = 0.62
    root.add(post)

    const torso = new Mesh(sharedBoxGeometry(0.42, 0.5, 0.24), cloth)
    torso.name = 'TorsoPad'
    torso.position.y = 0.92
    root.add(torso)

    const head = new Mesh(sharedSphereGeometry(0.18, 12, 8), straw)
    head.name = 'StrawHead'
    head.position.y = 1.32
    root.add(head)

    const arm = new Mesh(sharedCylinderGeometry(0.045, 0.045, 0.82, 8), wood)
    arm.name = 'CrossArm'
    arm.position.y = 1.02
    arm.rotation.z = Math.PI * 0.5
    root.add(arm)

    const base = new Mesh(sharedTorusGeometry(0.18, 0.035, 8, 20), wood)
    base.name = 'BaseRing'
    base.position.y = 0.06
    base.rotation.x = Math.PI * 0.5
    root.add(base)

    const cap = new Mesh(sharedConeGeometry(0.16, 0.18, 8), straw)
    cap.name = 'HeadWrap'
    cap.position.y = 1.5
    root.add(cap)
